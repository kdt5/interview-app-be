...
async function getUserFromToken(accessToken: string): Promise<UserInfo> {
  try {
...
async function deleteRefreshToken(userId: number): Promise<void> {
...
function mapUser(user: UserInfo): UserInfo {
  return {
    userId: user.userId,
    email: user.email,
